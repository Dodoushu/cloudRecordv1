...
void main() => runApp(MaterialApp(
      home: treatmentRecord(),
    ));

class treatmentRecord extends StatefulWidget {
  @override
  State createState() => new _treatmentRecord();
}

class _treatmentRecord extends State<treatmentRecord> {
  void initState() {
    super.initState();
    getId();
  }
...
//    验证Form表单
    if (loginForm.validate() &&(date!=null)&&
      ...
      DioManager.getInstance().post('OutPatient/TreatRecords', formData,
          (data) {
        Widget okButton = FlatButton(
          child: Text("好的"),
          onPressed: () {
        ...
        bottonList.add(okButton);
        showAlertDialog(context,
            titleText: '操作成功', contentText: '治疗记录上传成功', ButtonList: bottonList);
        print(data);
      }, (error) {
        print(error);
