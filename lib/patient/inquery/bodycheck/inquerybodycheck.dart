...
void main() {
  runApp(new MaterialApp(
    ...
    home: new InqueryBodycheck(),
  ));
}

class InqueryBodycheck extends StatefulWidget {
  @override
  _State createState() => new _State();
}

class _State extends State<InqueryBodycheck> {
  @override
  void initState() {
    super.initState();
...
//    getInfo();
  ...
  getDateInfo() async{
    ...
    DioManager.getInstance().post(
      'SelectExamineRecords',
      formData,
          (data) {
        list.clear();
        for (Map map in data['examineRecords']) {
          map['class'] = '体检记录';
          list.add(map);
        }
        print(list);
  ...
  getInfo() async{
    ...
    formData['sDate'] = sDate.toIso8601String().substring(0,10);
    formData['eDate'] = eDate.toIso8601String().substring(0,10);
    print(formData);
    DioManager.getInstance().post(
      'SelectExamineRecords',
      formData,
